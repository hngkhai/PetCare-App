...
import { View, Text, StyleSheet, TouchableOpacity,Image, Dimensions, ActivityIndicator, Alert, RefreshControl } from 'react-native';
...
const fetchUsers = async () => {
      setIsLoading(true);
        try {
            ...
            const response = await fetch('http://10.91.144.154:8080/api/adoption/getAdoptionByAdp/'+userId);
            if (!response.ok) {
           ...
           /*
            users.forEach(user => {
              console.log(`Pet Name: ${user.imagesList}, Age: ${user.age}, Type: ${user.type}`);
            }); */
